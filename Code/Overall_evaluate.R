...
first <- read_csv(grep("csv",list.files("~/Repositories/QFL_Bot/Data/trading_sessions/First",
                                        full.names = T),
                       value = T))
second <- read_csv(grep("csv",list.files("~/Repositories/QFL_Bot/Data/trading_sessions/Second",
                                        full.names = T),
                       value = T),col_types = cols(MESSAGE = col_character()))

third <- read_csv(grep("csv",list.files("~/Repositories/QFL_Bot/Data/trading_sessions/Third",
                                         full.names = T),
                        value = T),col_types = cols(MESSAGE = col_character()))
...
p1 <- ggplot(data=quote_equity, aes(x= closetm_SELL, y = cumul))+
  ...
  scale_x_date(date_labels="%d-%m-%Y",date_breaks  ="7 day")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  # theme(axis.text.x = element_blank())+
  theme(panel.grid.minor = element_blank())+
...
quote[, PAIR := "MYSTRATEGY"]
quote[,cum_diff_per := cumul/1150*100]
setnames(quote, c("closetm_SELL"), c("Date_POSIXct"))
quote$cumul <- NULL
quote$quote_result_clean <- NULL
df1 <- rbind(dfdf, quote)
df2 <- df1[Date_POSIXct <= df1[PAIR == "MYSTRATEGY", max(Date_POSIXct)]]
unique(df2$PAIR)[i]

path_alerts <- "Alerts"
pdf(paste0(path_alerts, "/Alpha.pdf"), onefile = TRUE)
i <- 1
for (i in 1:length(unique(df2$PAIR))){
  
  p<- ggplot(data = df2[PAIR %in% c(unique(df2$PAIR)[i], "MYSTRATEGY")], aes(x = Date_POSIXct, y = cum_diff_per, colour = PAIR))+
